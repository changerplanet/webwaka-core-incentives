...
export function evaluateIncentives(
  ...
  const activeDefinitions = definitions.filter(def => {
    ...
    if (def.tenantId !== context.tenantId) return false;
    
    if (def.validFrom && new Date(def.validFrom) > new Date(timestamp)) return false;
    if (def.validUntil && new Date(def.validUntil) < new Date(timestamp)) return false;
    
    return true;
  });

  for (const definition of activeDefinitions) {
    const campaignId = context.metadata?.campaignId as string | undefined;
    const rule = findApplicableRule(definition, rules, context.subjectId, campaignId);
    
    if (definition.type === 'multi_level') {
      const multiLevelResults = evaluateMultiLevelIncentive(
        definition,
        rules,
        relationships,
        context,
      ...
      const result = evaluateSingleIncentive(
        definition,
        rule,
        context,
        inputValue,
...
function evaluateSingleIncentive(
  ...
  let evaluation: { amount: number; calculation: string };
...
function evaluateMultiLevelIncentive(
  definition: IncentiveDefinition,
  rules: readonly IncentiveRule[],
  relationships: readonly IncentiveRelationship[],
  context: IncentiveEvaluationContext,
  ...
  for (const { referrerId, depth } of referralChain) {
    ...
    if (evaluation.amount <= 0) continue;
